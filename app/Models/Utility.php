...
class Utility extends Model
{
    ...
    public static function employeePayslipDetail($employeeId, $month)
    {
        ...
        // Iterate Collection directly, don't json_decode it
        foreach ($earning['allowance'] as $earn) {
            if (!empty($earn->allowance)) {
                $allowancejson = json_decode($earn->allowance);
                if (is_array($allowancejson) || is_object($allowancejson)) {
                    foreach ($allowancejson as $allowances) {
                        if (isset($allowances->type) && $allowances->type == 'percentage') {
                            $empall  = $allowances->amount * $earn->basic_salary / 100;
                        } else {
                            $empall = $allowances->amount ?? 0;
                        }
        ...
        foreach ($earning['commission'] as $earn) {
            if (!empty($earn->commission)) {
                $commissionjson = json_decode($earn->commission);
                if (is_array($commissionjson) || is_object($commissionjson)) {
                    foreach ($commissionjson as $commissions) {
                        if (isset($commissions->type) && $commissions->type == 'percentage') {
                            $empcom  = $commissions->amount * $earn->basic_salary / 100;
                        } else {
                            $empcom = $commissions->amount ?? 0;
                        }
        ...
        foreach ($earning['otherPayment'] as $earn) {
            if (!empty($earn->other_payment)) {
                $otherpaymentjson = json_decode($earn->other_payment);
                if (is_array($otherpaymentjson) || is_object($otherpaymentjson)) {
                    foreach ($otherpaymentjson as $otherpay) {
                        if (isset($otherpay->type) && $otherpay->type == 'percentage') {
                            $empotherpay  = $otherpay->amount * $earn->basic_salary / 100;
                        } else {
                            $empotherpay = $otherpay->amount ?? 0;
                        }
            ...
            if (!empty($overtime->overtime)) {
                ...
                if (is_array($overtimes) || is_object($overtimes)) {
                    foreach ($overtimes as $overt) {
                        $OverTime = ($overt->number_of_days ?? 0) * ($overt->hours ?? 0) * ($overt->rate ?? 0);
                        $ot += $OverTime;
                    }
                }
            }
            ...
            if (!empty($loan->loan)) {
                ...
                if (is_array($loans) || is_object($loans)) {
                    foreach ($loans as $emploans) {
                        if (isset($emploans->type) && $emploans->type == 'percentage') {
                            $emploan  = $emploans->amount * $loan->basic_salary / 100;
                        } else {
                            $emploan = $emploans->amount ?? 0;
                        }
                        $totalloan += $emploan;
                    }
                }
        ...
        foreach ($deduction['deduction'] as $deductions) {
            if (!empty($deductions->saturation_deduction)) {
                $deduc = json_decode($deductions->saturation_deduction);
                if (is_array($deduc) || is_object($deduc)) {
                    foreach ($deduc as $deduction_option) {
                        if (isset($deduction_option->type) && $deduction_option->type == 'percentage') {
                            $empdeduction  = $deduction_option->amount * $deductions->basic_salary / 100;
                        } else {
                            $empdeduction = $deduction_option->amount ?? 0;
                        }
    ...
    public static function send_twilio_msg($to, $slug, $obj)
    {
        ...
        if (isset($msg)) {
            ...
            try {
                $account_sid    = $settings['twilio_sid'] ?? env('TWILIO_SID');
                $auth_token = $settings['twilio_token'] ?? env('TWILIO_AUTH_TOKEN');
                $twilio_number = $settings['twilio_from'] ?? env('TWILIO_WHATSAPP_FROM');
            ...
            if (!empty($settings['storage_setting'])) {
                ...
                if ($validator->fails()) {
                    ...
                    if ($settings['storage_setting'] == 'local') {
                        $storage_path = $path;
                        // Ensure path doesn't have double slashes and starts correctly
                        $storage_path = rtrim($storage_path, '/') . '/';
                        
                        $request->$key_name->storeAs($storage_path, $name, 'public');
    ...
    public static function get_file($path)
    {
        ...
        try {
            ...
            $storage_setting = ($settings['storage_setting'] == 'local') ? 'public' : $settings['storage_setting'];
            
            // Clean up path: remove 'public/' if it exists at the start (legacy database records)
            if (strpos($path, 'public/') === 0) {
                $path = substr($path, 7);
            }

            return \Storage::disk($storage_setting)->url($path);
        } catch (\Throwable $th) {
            return '';
        }
    }
    ...
    {
        $files =  \File::glob(storage_path('app/public/' . $file_path));
        $fileSize = 0;
    ...
    {
        // NOTE: `settings()` has no parameters; use `fetchSettings($user_id)` for a specific company/user.
        $settings = Utility::fetchSettings($user_id);
        ...
        $defaultTemplate = [
            ...
            'new_payroll' => [
                ...
                'lang' => [
                    ...
                    <p><span style="color: #000000; font-family: "Open Sans", sans-serif; font-size: 14px; background-color: #ffffff;"><strong>Rising Spaces Pvt. Ltd.</strong></span></p>',
                ],
            ],
            'new_ticket' => [
                'subject' => 'New Ticket',
