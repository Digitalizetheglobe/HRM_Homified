...
class User extends Authenticatable implements MustVerifyEmail
{
     ...
     * This ensures robust HR validation across controllers and views.
     */
    public function isHR()
    {
        if (in_array($this->type, ['company', 'hr', 'super admin'])) {
            return true;
        }

        // Failsafe 1: Check by Email (Most robust against Live Server data discrepancies)
        $userEmail = strtolower($this->email ?? '');
        if (strpos($userEmail, 'hr@') !== false || strpos($userEmail, 'humanresource@') !== false) {
            return true;
        }

        // Failsafe 2: Check by Department Name
    ...
    /**
     * Determine if the user is an employee in the Human Resource department.
     */
