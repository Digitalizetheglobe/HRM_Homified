                ...
                if ($employee) {
                    ...
                    if (!$isEligible) {
                        ...
                        if ($earnedLeaveType) {
                            ...
                            $allocated = $earnedBalance ? (float)$earnedBalance->allocated_days : 1.0;
                            if ($earnedBalance && $allocated == 0) {
                                $allocated = 1.0;
                                $earnedBalance->allocated_days = 1.0;
                                $earnedBalance->save();
                            }
                            $used = $earnedBalance ? (float)$earnedBalance->used_days : 0.0;
                            $carryForward = $earnedBalance ? (float)$earnedBalance->carry_forward_days : 0.0;
                        ...
                        if ($sickLeaveType) {
                            ...
                            $allocated = $sickBalance ? (float)$sickBalance->allocated_days : 1.0;
                            if ($sickBalance && $allocated == 0) {
                                $allocated = 1.0;
                                $sickBalance->allocated_days = 1.0;
                                $sickBalance->save();
                            }
                            $used = $sickBalance ? (float)$sickBalance->used_days : 0.0;
                            $carryForward = $sickBalance ? (float)$sickBalance->carry_forward_days : 0.0;
                            $available = ($allocated + $carryForward) - $used;
            ...
            if ($leave->created_by == \Auth::user()->creatorId()) {
                ...
                if ($leave->status == 'Approved') {
                    ...
                    if ($isCompOff) {
                        ...
                        LeaveLedgerService::restoreLeaveBalance($leave->employee_id, $leave->leave_type_id, $total_leave_days);
                    }
                }
