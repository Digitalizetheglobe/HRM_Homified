                    ...
                    foreach($decode_content as $key => $value)
                    {
                        ...
                        if($key == "logo")
                        {
                            if($request->hasFile('logo'))
                            {
                                ...
                                $request->file('logo')->storeAs('uploads/custom_landing_page_image', $fileName, 'public');
                                $data['logo'] = $fileName;
                            }
                            else
                            {
                        ...
                        else if($key == "image")
                        {
                            if($request->hasFile('image'))
                            {
                                ...
                                $request->file('image')->storeAs('uploads/custom_landing_page_image', $fileName, 'public');
                                $data['image'] = $fileName;
                            }
                            else
                            {
                                ...
                                foreach($image_array as $image_array_key => $image_array_val)
                                {
                                    foreach($value as $val_key => $val_data)
                                    {
                                        if($val_data->id == $image_array_key)
                                        {
                                            ...
                                            $image_array_val->storeAs('uploads/custom_landing_page_image', $fileName, 'public');
                                                        ...
                                                        foreach($json_data as $contact_key => $contact_data)
                                                        {
                                                            if($val['id'] == $contact_data->id)
                                                            {
                                                                if(!empty($val['image']))
                                                                {
                                                                    ...
                                                                    $val['image']->storeAs('uploads/custom_landing_page_image', $fileName, 'public');
