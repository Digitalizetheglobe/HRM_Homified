...
class TicketController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'em' || (\Auth::user()->can('ticket.manage.view.all') && $own != 1)) {
        ...
        if (\Auth::user()->type == 'company' || \Auth::user()->can('ticket.manage.view.all') || \Auth::user()->can('ticket.manage.view.own') || \Auth::user()->type == 'employee') {
            $user = Auth::user();
            if ($own == 1 || ($user->type == 'employee' && !$user->can('ticket.manage.view.all'))) {
                $tickets = Ticket::where('employee_id', '=', \Auth::user()->id)->orWhere('ticket_created', \Auth::user()->id)->orderBy('id', 'desc')->get();
            } else {
                $tickets = Ticket::select('tickets.*')->join('users', 'tickets.created_by', '=', 'users.id')->where('users.created_by', '=', \Auth::user()->creatorId())->orWhere('tickets.created_by', \Auth::user()->creatorId())->orderBy('tickets.id', 'desc')->get();
            }
    ...
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->can('ticket.manage.create.all') || \Auth::user()->can('ticket.manage.create.own') || \Auth::user()->type == 'employee') {
    ...
    public function edit($ticket)
    {
        ...
        if (\Auth::user()->type == 'company' || \Auth::user()->can('ticket.manage.edit.all') || (\Auth::user()->type == 'employee' && \Auth::user()->can('ticket.manage.edit.own'))) {
            $employees = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->get()->pluck('name', 'id');
    ...
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->can('ticket.manage.delete.all') || (\Auth::user()->type == 'employee' && \Auth::user()->can('ticket.manage.delete.own'))) {
