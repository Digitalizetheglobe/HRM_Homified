...
class BranchController extends Controller
{
    ...
    {
        $user = \Auth::user();
        if ($user->can('Manage Branch') || $user->can('setup.hrm.view.company')) {
    ...
    public function getemployee(Request $request)
    {
        ...
        if (in_array('0', $request->department_id)) {
            $employees = $query->get()->pluck('name', 'id')->toArray();
        } else {
            $employees = $query->whereIn('department_id', $request->department_id)->get()->pluck('name', 'id')->toArray();
        }
