    ...
    public function index()
{
    ...
    public function editprofile(Request $request)
    {
        ...
        $validator = \Validator::make(
            ...
            [
                ...
                // Employees cannot edit their email - only non-employee users can
                'email' => (\Auth::user()->type !== 'employee') ? 'required|email|unique:users,email,' . $userDetail['id'] : '',
                // 'profile' => 'required',
            ]
        );
        if ($validator->fails()) {
        ...
        if ($request->hasFile('profile')) {
            ...
            $storage_setting = !empty($settings['storage_setting']) ? $settings['storage_setting'] : 'local';
            
            // Delete old avatar if exists
            if ($userDetail['avatar']) {
                if ($storage_setting == 'local') {
                    // Use 'public' disk for local storage to make files accessible via public/storage symlink
                    \Storage::disk('public')->delete($dir . $userDetail['avatar']);
                } else {
                    // For S3/Wasabi, use Storage disk
                    \Storage::disk($storage_setting)->delete($dir . $userDetail['avatar']);
                }
            }
            
            // Store new avatar
            if ($storage_setting == 'local') {
                // Use 'public' disk for local storage so files are accessible without running storage:link
                $path = $request->file('profile')->storeAs($dir, $fileNameToStore, 'public');
                $user->avatar = $fileNameToStore;
            } else {
                // Use Utility::upload_file for S3/Wasabi storage
        ...
        // Only allow email updates for non-employee users
        if (\Auth::user()->type !== 'employee' && !empty($request['email'])) {
            $user->email = $request['email'];

            // Also update employee email for non-employee users editing employee profiles
    ...
    public function markAllRead()
    {
        ...
        // Mark manual Leave notifications as seen
        if ($user->type == 'company') {
            \App\Models\Leave::where('created_by', $user->creatorId())
                ->where('seen_by_manager', 0)
                ->update(['seen_by_manager' => 1]);
        } elseif (in_array(strtolower($user->type), ['director', 'hr'])) {
            \App\Models\Leave::where('forwarded_to_director_id', $user->id)
                ->where('seen_by_director', 0)
                ->update(['seen_by_director' => 1]);
        }
