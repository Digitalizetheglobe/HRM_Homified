...
@if ($isCompanyUser)
    <div class="modal-footer">
        {{-- Always show Approve/Reject buttons for Company User, even if forwarded --}}
        @if($isPending)
            <input type="submit" value="{{ __('Approved') }}" class="btn btn-success rounded" name="status" id="approve-btn">
            <input type="submit" value="{{ __('Reject') }}" class="btn btn-danger rounded" name="status">
            @if(!$leave->forwarded_to_director_id && $directors->count() > 0)
                <button type="button" class="btn btn-primary rounded" id="forward-btn" onclick="showForwardSection()">{{ __('Approve & Forward') }}</button>
            @endif
            @if($leave->forwarded_to_director_id)
                <small class="text-muted d-block mt-2">{{ __('This leave has been forwarded for director/HR approval. You can still approve or reject.') }}</small>
            @endif
        @else
            {{-- Leave is already approved or rejected --}}
            <p class="text-muted mb-0">{{ __('Leave status: ') . $leave->status }}</p>
        @endif
    </div>
@elseif($isDirectorUser && $isForwardedToUser && $isCompanyApproved && !$leave->director_approved)
    {{-- Director can approve forwarded leaves that are company approved --}}
    <div class="modal-footer">
        <input type="submit" value="{{ __('Approved') }}" class="btn btn-success rounded" name="status">
        <input type="submit" value="{{ __('Reject') }}" class="btn btn-danger rounded" name="status">
    </div>
@elseif($isDirectorUser)
    {{-- Director viewing other leaves - no action buttons --}}
    <div class="modal-footer">
        <p class="text-muted mb-0">{{ __('You can only approve leaves that are forwarded to you by company users.') }}</p>
    </div>
@elseif($isHrUser && $isForwardedToUser && $isCompanyApproved && !$leave->director_approved)
    <div class="modal-footer">
        <input type="submit" value="{{ __('Approved') }}" class="btn btn-success rounded" name="status">
        <input type="submit" value="{{ __('Reject') }}" class="btn btn-danger rounded" name="status">
    </div>
