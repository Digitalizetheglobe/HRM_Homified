...
@section('page-title')
    {{ __('Role & Permissions Assignment') }}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Home') }}</a></li>
    <li class="breadcrumb-item">{{ __('Employee Permissions') }}</li>
@endsection
...
<div class="row" x-data="permissionManager()">
    <div class="col-12 mb-4">
        <div class="card shadow-sm border-0">
            <div class="card-body">
                <form id="permissionForm" method="POST" action="{{ route('employee-permissions.sync') }}">
                    @csrf
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <label class="form-label text-muted fw-bold">{{ __('Select Employee to Configure') }}</label>
                            <select name="user_id" class="form-control" x-model="selectedEmployee" @change="fetchPermissions()" x-init="new Choices($el, { searchEnabled: true, removeItemButton: true, itemSelectText: '' })">
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ session('selected_user_id') == $employee->id ? 'selected' : '' }}>{{ $employee->name }} ({{ $employee->type }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 text-end mt-4">
                            <button type="submit" class="btn btn-primary px-4 shadow-sm" :disabled="!selectedEmployee">
                                <i class="ti ti-device-floppy me-2"></i>{{ __('Save Permissions') }}
                            </button>
                        </div>
                    </div>
            ...
            <div class="col-md-3">
                <div class="card shadow-sm border-0" style="min-height: 600px; position: sticky; top: 100px; z-index: 10;">
                    <div class="card-header bg-white border-bottom-0 pt-4 pb-0">
                        <h6 class="text-uppercase text-muted fw-bold mb-0" style="font-size: 12px; letter-spacing: 1px;">{{ __('Modules') }}</h6>
                    </div>
                    <div class="card-body p-3">
                        <div class="list-group list-group-flush rounded-3">
                            @foreach($menuPermissions as $moduleName => $features)
                                <a href="#" data-module-name="{{ $moduleName }}"
                                   ...
                                   :class="{ 'bg-primary text-white shadow-sm': activeModule === '{{ $moduleName }}', 'text-dark bg-light': activeModule !== '{{ $moduleName }}' }"
                                   @click.prevent="setActiveModule('{{ $moduleName }}', {{ json_encode($features) }})">
                                    <span class="fw-medium">
                                        <i class="ti ti-box me-2" :class="{ 'text-white': activeModule === '{{ $moduleName }}', 'text-primary': activeModule !== '{{ $moduleName }}' }"></i> 
                                        {{ $moduleName }}
                                    </span>
            ...
            <div class="col-md-9">
                <div class="card shadow-sm border-0" style="min-height: 600px;">
                    <div class="card-header bg-white border-bottom pt-4 pb-3">
                        <h5 class="mb-0 text-primary fw-bold" x-text="activeModuleName ? activeModuleName + ' Configuration' : '{{ __('Select a module from the left') }}'"></h5>
                        <p class="text-muted small mb-0 mt-1" x-show="activeModuleName">{{ __('Configure Own vs Employees permissions for each feature.') }}</p>
                    </div>
                    ...
                    <div class="card-body p-4 bg-light">
                        <div x-show="!activeModuleName" class="text-center text-muted mt-5">
                            ...
                            <h5 class="fw-normal">{{ __('No Module Selected') }}</h5>
                            <p class="mt-2">{{ __('Please select a module from the sidebar to configure its permissions.') }}</p>
                        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('permissionManager', () => ({
            selectedEmployee: '{{ session('selected_user_id', '') }}',
            activeModule: '',
            activeModuleName: '',
            activeModuleData: {},
            ...
            fetchPermissions() {
                ...
                fetch('{{ route('employee-permissions.fetch') }}', {
                    method: 'POST',
                    headers: {
